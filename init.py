...
def viewFlightCustomer():
    ...
    cursor.close()
    if (data):
        return render_template('viewFlightCustomer.html', username=username, fromdate=fromdate, todate=todate,
                               posts=data)
    else:
...
def trackMySpending():
    username = session['username']
    cursor = conn.cursor()
    query = f'SELECT sum(price) as total \
        FROM purchases, ticket, flight \
        WHERE purchases.ticket_id = ticket.ticket_id \
    ...
    monthlySpending = ''
    for i in range(6):
        query = f'SELECT sum(price) as monthlySpending \
            FROM purchases, ticket, flight \
            WHERE purchases.ticket_id = ticket.ticket_id \
            AND ticket.airline_name = flight.airline_name AND ticket.flight_num = flight.flight_num \
        ...
        monthlySpending += str(data[0][0]) + ' ' + str(month) + ','
    cursor.close()
...
def trackSpendingSpecific():
    ...
    # Six month bar chart data
    monthlySpending = ''
    month_difference = (int(todate[:4]) - int(fromdate[:4])) * 12 + int(todate[5:7]) - int(fromdate[5:7])
    print(month_difference)
    for i in range(month_difference + 1):
        query = 'SELECT sum(price) as monthlySpending \
            FROM purchases, ticket, flight \
            WHERE purchases.ticket_id = ticket.ticket_id \
            AND ticket.airline_name = flight.airline_name AND ticket.flight_num = flight.flight_num \
            ...
            AND purchases.customer_email = %s'
        cursor.execute(query, (todate, todate, username))
        data = cursor.fetchall()
        month = ((int(todate[5:7]) - (i + 1)) % 12) + 1
        monthlySpending += str(data[0][0]) + ' ' + str(month) + ','

    cursor.close()
    return render_template('trackSpending.html', fromdate=fromdate, todate=todate, total=totalMoney[0],
                           results=monthlySpending)
...
def purchaseAgent():
    ...
    airline_name = request.form['airline_name']
    flight_num = request.form['flight_num']
    # Verify if the agent works for this airline whose flight is to be bought
    cursor = conn.cursor()
    airline = request.form['airline_name']
    query = 'SELECT * FROM booking_agent_work_for WHERE email = "%s"'
    ...
    if (data):
        ...
        error = 'No permission from this airline'
        return render_template('purchaseAgent.html', searchError=error)
...
def searchAgent():
    ...
    toairport = request.form['toairport']
    todate = request.form['todate']
    queryGetID = f'SELECT booking_agent_id FROM booking_agent WHERE email="{username}"'
...
def viewMyCommission():
    ...
    queryGetCommission = f'SELECT sum(price)*.10 as totalComm FROM purchases, ticket, flight \
                            WHERE purchases.ticket_id = ticket.ticket_id \
                            AND ticket.airline_name = flight.airline_name AND ticket.flight_num = flight.flight_num \
                            AND purchases.purchase_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND CURDATE() \
                            AND purchases.booking_agent_id = "{username}"'
    cursor.execute(queryGetCommission)
    totalComm = cursor.fetchone()
    totalCommVal = totalComm[0] or 0
    # Get total tickets in the past 30 days 
    queryGetTicketCount = f'SELECT count(*) as ticketCount FROM purchases, ticket, flight \
                            WHERE purchases.ticket_id = ticket.ticket_id \
                            AND ticket.airline_name = flight.airline_name AND ticket.flight_num = flight.flight_num \
                            AND purchases.purchase_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND CURDATE() \
                            AND purchases.booking_agent_id = "{username}"'
    cursor.execute(queryGetTicketCount)
    ticketCount = cursor.fetchone()
    ...
    cursor.close()
    return render_template('viewCommissionAgent.html', username=username, totalComm=totalCommVal, avgComm=avgComm,
...
def commission():
    ...
    cursor.execute(queryGetID % (username))
    agentID = cursor.fetchone()
    # Get total commssion in the given period, ratio 0.1
    ...
    queryGetTicketCount = f'SELECT count(*) as ticketCount FROM purchases, ticket, flight \
                            WHERE purchases.ticket_id = ticket.ticket_id \
                            AND ticket.airline_name = flight.airline_name AND ticket.flight_num = flight.flight_num \
                            AND purchases.purchase_date BETWEEN CAST({fromdate} AS DATE) AND CAST({todate} AS DATE) \
                            AND purchases.booking_agent_id = "{username}"'
    cursor.execute(queryGetTicketCount)
    ticketCount = cursor.fetchone()
    ticketCountVal = ticketCount[0]
    cursor.close()
    return render_template('commission.html', fromdate=fromdate, todate=todate, totalComm=totalCommVal,
                           ticketCount=ticketCountVal)
...
def viewTop5Customers():
    username = session['username']
    cursor = conn.cursor()
    query1 = f'SELECT customer_email, count(ticket_id) as ticket_sales FROM booking_agent NATURAL JOIN purchases ' \
             f'WHERE (purchase_date between date_sub(curdate(), interval 6 month) and curdate()) and ' \
             f'email = "{username}" group by customer_email order by ticket_sales DESC limit 5'
    cursor.execute(query1)
    data1 = cursor.fetchall()
    ticketTop5 = ''
    for i in data1:
        ticketTop5 += str(i[1]) + " " + str(i[0]) + ","
    query2 = f'SELECT customer_email, sum(flight.price) as commission\
                ...
                WHERE (purchase_date between date_sub(curdate(), interval 1 year) and curdate())\
                    ...
                    AND ticket.airline_name = flight.airline_name \
                    AND ticket.flight_num = flight.flight_num \
                    AND booking_agent.email = "{username}"\
...
def staffvalidation():
    ...
    cursor = conn.cursor()
    query = f'select * from airline_staff where username="{username}"'
    cursor.execute(query)
    data = cursor.fetchall()
    cursor.close()
    if data:
...
def staffsearchresult_filter():
    if staffvalidation():
        ...
        error = None
        if data:
            return render_template('staffsearchflight.html', username=username, results=data,
                                   message='Here is the filter result:')
        else:
            # returns an error message to the html page
            error = 'No results found'
            return redirect(url_for('staffHome', error=error))
...
def staffsearchresult_customers():
    if staffvalidation():
        ...
        cursor.close()
        if data:
            return render_template('staffsearchflight.html', username=username, customerresults=data,
                                   message='Customers on Flight ' + flightnum)
        else:
            # returns an error message to the html page
            error = 'No results found'
            return redirect(url_for('staffHome', error=error))
...
def createflight():
    if staffvalidation():
        ...
        cursor.execute(
            query % (airline, flightnum, departport, departtime, arriveport, arrivetime, price, status, airplaneid))
        conn.commit()
        cursor.close()
...
def viewreportresult1():
    if staffvalidation():
        ...
        return render_template('viewreport.html',
                               message='Tickets sold between ' + begindate + ' and ' + enddate + ' is ' + str(
                                   data1[0][0]) + '.', results=yearlyticketsales)
...
def viewreportresult2():
    if staffvalidation():
        ...
        return render_template('viewreport.html',
                               message='Tickets sold last ' + period + ' is ' + str(data1[0][0]) + '.',
                               results=yearlyticketsales)
...
def viewrevenues():
    if staffvalidation():
        ...
        return render_template('viewrevenue.html', mdirect=data1[0][0], mindirect=data2[0][0], ydirect=data3[0][0],
                               yindirect=data4[0][0])
    else:
        error = 'Invalid credentials, please login again!!'
        return redirect(url_for('hello', error=error))
...
@app.route('/staffHome/viewGrantNewPermissions')
def viewGrantNewPermissions():
    error = request.args.get('error')
    return render_template('viewGrantNewPermissions.html', error=error)


@app.route('/GrantPermission', methods=['POST'])
def GrantPermission():
    # Give permission to a staff in this airline, and verify if the staff is indeed from this airline!
    username = session['username']  # admin
    airline = staffairline()
    new_permission = request.form['username']  # give permission to
    cursor = conn.cursor()
    query0 = 'select * from airline_staff where username = "%s"'
    cursor.execute(query0 % new_permission)
    data = cursor.fetchone()
    if not data:
        error = 'User does not exist'
        return redirect(url_for('viewGrantNewPermissions', error=error))
    query1 = 'select * from airline_staff where username = "%s" and airline_name = "%s"'
    cursor.execute(query1 % (new_permission, airline))
    data = cursor.fetchone()
    cursor.close()
    if data:
        cursor = conn.cursor()
        query2 = 'update permission set permission_type = "admin" where username = "%s"'
        cursor.execute(query2 % new_permission)
        cursor.close()
        return redirect('/staffHome')
    else:
        error = 'No person in airline'
        return redirect(url_for('viewGrantNewPermissions', error=error))


@app.route('/staffHome/viewAddBookingAgents')
def viewAddBookingAgents():
    return render_template('viewAddBookingAgents.html')


@app.route('/AddBookingAgents', methods=['POST'])
def AddBookingAgents():
    username = session['username']
    new_agent_email = request.form['username']
    cursor = conn.cursor()
    query = 'SELECT * FROM booking_agent_work_for WHERE email = "%s"'
    cursor.execute(query % new_agent_email)
    data = cursor.fetchone()
    cursor.close()
    if data:
        cursor = conn.cursor()
        ...
        airline = cursor.fetchone()[0]
        query1 = 'INSERT INTO booking_agent_work_for VALUES ("%s", "%s")'
        cursor.execute(query1 % (new_agent_email, airline))
        cursor.close()
        return redirect('/staffHome')
    else:
        error = 'Already exists'
        return render_template('viewAddBookingAgents.html', error=error)
